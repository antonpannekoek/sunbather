...
def save_temp_parker_profile(
    planet, Mdot, T, zdict, pdir, mu_bar, mu_struc=None, no_tidal=False, altmax=20
):
    """
    Uses the p-winds code (dos Santos et al. 2022)
...
def save_cloudy_parker_profile(
    ...
    filename, launch_velocity = save_temp_parker_profile(
        planet,
        Mdot,
        T,
        zdict,
    ...
    tools.verbose_print(
        f"Saved temp parker profile with neutral mu_bar: {neutral_mu_bar}",
        verbose=verbose,
    )
    ...
    for itno in range(maxit):
        ...
        tools.verbose_print(
            f"Making new parker profile with p-winds based on Cloudy's reported mu_bar: {mu_bar}",
            verbose=verbose,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-pdir",
        required=True,
        help="directory where the profiles are saved. It is adviced to choose a name that "
    ...
    composition_group.add_argument(
        "-fH",
        type=float,
        help="hydrogen fraction by number. Using this command results in running standalone p_winds without invoking Cloudy.",
    )
    composition_group.add_argument(
        "-z",
        type=float,
        help="metallicity (=scale factor relative to solar for all elements except H and He). Using this "
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="relative change in mu allowed for convergence, when using p_winds/Cloudy iterative scheme [default=0.01]",
    )
    parser.add_argument(
        "-mu_maxit",
        type=int,
        default=7,
        help="maximum number of iterations the p_winds/Cloudy iterative scheme is ran "
        "if convergence is not reached [default =7]",
    )
    parser.add_argument(
        "-verbose",
        action="store_true",
        help="print out mu-bar values of each iteration [default=False]",
    )
    parser.add_argument(
        "-no_tidal",
        action="store_true",
        help="neglect the stellar tidal gravity term [default=False, i.e. tidal term included]",
    )
    ...
    if args.fH != None and (
        args.zelem != {} or args.mu_conv != 0.01 or args.mu_maxit != 7
    ):
        warnings.warn(
            "The -zelem, -mu_conv, and -mu_maxit commands only combine with -z, not with -fH, so I will ignore their input."
        )
